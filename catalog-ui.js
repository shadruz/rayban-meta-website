(function(){
 ...
 function select(select){
  ...
  container.querySelectorAll('[data-price]').forEach(n=>{n.dataset.price=p.id;n.textContent=(p.price===null?'$???':'$'+p.price);});
  ...
  const price=container.querySelector('[data-detail-price]');if(price)price.textContent=(p.price===null?'$???':'$'+p.price);
  ...
  else if(status)status.textContent=lang()==='ru'?'Условия поставки выбранной комплектации уточняются перед оформлением.':'Buyurtma asosida. Xarid imkoniyati va muddat oldindan tasdiqlanadi.';
 ...
 function filter(){
  ...
  const ordered=[...cards];if(sort!=='default')ordered.sort((a,b)=>(a.dataset.minPrice==='')-(b.dataset.minPrice==='')||(Number(a.dataset.minPrice)-Number(b.dataset.minPrice))*(sort==='desc'?-1:1));
